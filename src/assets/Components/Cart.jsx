...
const Cart = ({ cartData }) => {
  ...
  return (
    <div className='max-h-screen overflow-y-hidden w-screen bg-white'>
      <Navbar />
      ...
      {cartData.map((elem, idx) => {
        ...
        return (
          <div key={idx} className='h-fit w-screen px-5 flex items-center justify-center'>
            <div className="w-full h-auto flex flex-col items-start space-y-4 border border-gray-300 p-5 rounded-lg shadow-lg">
              <div className="flex items-center w-full">
                <img className='h-[100px] w-[100px] bg-[#f3f4f7]' src={elem.img} alt="img" />
      ...
      <button className='w-screen h-[63px] bg-[#7c71df] absolute bottom-0 text-white font-normal text-[20px] leading-[30.2px] text-center'>Next</button>
    </div>
  );
}
